...
class CreateComponent(FlaskForm):
    ...
    movements = SelectMultipleField('movements', choices=[(
        movement, movement) for movement in Movements.query.order_by(Movements.name).all()], validators=[DataRequired()])
    score_type = SelectField(
...
class CreateMovement(FlaskForm):
    ...
    kind = SelectField('movement type', choices=[('Weightlifting', 'weightlifting '), (
        'Gymnastics', 'gymnastics'), ('Monostructural', 'monostructural')])
    submit = SubmitField('submit')
